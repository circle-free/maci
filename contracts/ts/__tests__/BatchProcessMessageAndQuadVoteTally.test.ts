...
describe.only('BatchProcessMessage', () => {
    beforeAll(async () => {
        // Deploy contracts
    ...
    describe.only('Sign-ups', () => {
        it('The state root should be correct after signing up some users', async () => {
            ...
            for await (const user of users) {
                ...
                console.log('Signup Gas', receipt.gasUsed.toNumber())
            }
    ...
    describe.only('Publish messages', () => {
        it('The message root should be correct after publishing one message per user', async () => {
            ...
            for (const user of users) {
                ...
                const receipt = await tx.wait()
                console.log('Message Gas', receipt.gasUsed.toNumber())
    ...
    describe.only('Process messages', () => {
        it('the blank state leaf hash should match the one generated by the contract', async () => {
            ...
            const blankStateLeaf = StateLeaf.genBlankLeaf(emptyVoteOptionTreeRoot)
            expect(onChainHash.toString()).toEqual(blankStateLeaf.hash().toString())

            const onChainEmptyOptionRoot = await maciContract.emptyVoteOptionTreeRoot()
            expect(onChainEmptyOptionRoot.toString()).toEqual(emptyVoteOptionTreeRoot.toString())
        })
        ...
        it('batchProcessMessage should verify a proof and update the stateRoot', async () => {
            ...
            console.log('stateRoot.toString() ok')
        })
    })
